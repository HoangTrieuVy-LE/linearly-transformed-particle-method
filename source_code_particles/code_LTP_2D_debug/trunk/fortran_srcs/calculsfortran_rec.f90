...
!-------------------------------------------------------
    subroutine set_degre_spline(p)
        ...
        else if (phi_radial == 2) then ! (phi is gaussian )                               "
           radius_phi = 3.
        end if 
   end subroutine
...
!-------------------------------------------------------
    subroutine phi(y,x)
        ...
        if (phi_radial == 1) then ! phi is a spline
            ...
            print *,"ERROR : phi_radial value must be either 0 or 1 or 2. Current phi_radial = " , phi_radial 
        end if
    end subroutine
    !1D phi derivative
    ...
    subroutine second_derivative_phi(scd_deriv,x)
        ...
        if (phi_radial == 1) then
           ...
           scd_deriv = (-1 + x**2) * (1/sqrt(2*pi)) * exp(-(x**2) / 2.)
        else
            print *,"ERROR : phi_radial value must be either 0 or 1 or 2. Current phi_radial = " , phi_radial 
        end if
    end subroutine            
    ...
    subroutine phi_2d(phi_out,x,y)
        ...
        if ((phi_radial == 1) .OR. (phi_radial == 2)) then 
            ...
            print *,"ERROR : phi_radial value must be either 0 or 1. Current phi_radial = " , phi_radial 
        end if
    end subroutine
    ...
    subroutine grad_phi_2d(grad,x,y)
        ...
        if ((phi_radial == 1) .OR. (phi_radial == 2)) then 
            ...
            if (x**2 + y**2 >=radius_phi) then
                ...
                grad(2)= (1./cst_int_phi_exp) * (-2*y/(radius_phi-(x**2+y**2))**2) * exp(-1/(radius_phi-x**2-y**2))
            end if            
		else
            print *,"ERROR : phi_radial value must be either 0 or 1. Current phi_radial = " , phi_radial
        end if
    ...
    subroutine grad_phi_2d_h(grad_h, x, y, hx_remap, hy_remap)
        ...
        if ((phi_radial == 1) .OR. (phi_radial == 2)) then 
            ...
            call grad_phi_2d(grad_h, x/hx_remap , y/hy_remap)
            grad_h(1) = grad_h(1) /(hy_remap * hx_remap**2)
            grad_h(2) = grad_h(2) /(hx_remap * hy_remap**2)
		else
            print *,"ERROR : phi_radial value must be either 0 or 1. Current phi_radial = " , phi_radial
        end if
    end subroutine
    ...
    subroutine hess_phi_2d_h(hess_h, x, y, hx_remap, hy_remap)
        ...
        if ((phi_radial == 1) .OR. (phi_radial == 2)) then 
            ...
            hess_h(1,2) = 1./(hx_remap**2 * hy_remap**2)  * hess_h(1,2)
            hess_h(2,1) = 1./(hx_remap**2 * hy_remap**2) * hess_h(2,1)
...
!~        !$OMP END PARALLEL DO        
    ...
    !__________________________________________________________________!
    !rec_ltp_grad_at_coordinate(grad_rho_hn, Xpart, M, DD, hx_remap, hy_remap, Xpart(:,j), N_part)
    subroutine rec_ltp_grad_at_coordinate(grad_rho_hn, Xpart, M, DD, hx_remap, hy_remap,X_coord, N_part, print_log_in)
        ...
        double precision, dimension (2) :: tmp_val, tmp_val_2, tmp_val_3
        grad_rho_hn=0.
        grad_phi_hn=0.
...
!~ 					tmp_val = det_Dkn * M(k) * MATMUL(TRANSPOSE(Dkn), grad_phi_hn)
!~ 					open (unit=out_unit,file="ltp.log",action="write", position="append")
!~ 		            write (out_unit,*) k, X_coord, tmp_val, (hx_remap**3) * grad_phi_hn
!~ 	            close (out_unit)            	
!~                 end if                
...
!~       !$OMP END PARALLEL DO
    ...
    subroutine rec_ltp_on_convolution_grid(rho_ltp_on_conv_grid, Xpart, M, DD, Xgrid_convol, Ygrid_convol, hx_remap, hy_remap, &
    nqx, nqy, N_part)
      integer, intent(in)                                   :: N_part, nqx, nqy      
      double precision, dimension (2,N_part), intent(in)    :: Xpart
      double precision, dimension (N_part), intent(in)      :: M
      double precision, dimension (4, N_part), intent(in)   :: DD
      ...
      double precision, intent(in)                          :: hx_remap, hy_remap
      double precision, dimension (nqx,nqy), intent(out)    :: rho_ltp_on_conv_grid
...
!~         end do
        ...
        rayonk = max(hx_remap,hy_remap)/det_Dk*max(abs(d)+abs(b),abs(c)+abs(a))
        qkx = int(radius_phi * rayonk / deltax) + 1
        qky = int(radius_phi * rayonk / deltay) + 1
...
!-------------------------------------------------------    
    subroutine rec_sp_grad_at_coordinate(grad_rho, Xpart, M, epsi, X_coord, N_part)    
        ...
        double precision, dimension (2) :: tmp_val, tmp_val_2
        integer, parameter :: out_unit=20
        ...
        do k=1,N_part
            ...
            if ( (abs(X_coord(1) - 0.5625) <=0.000001) .AND. (abs(X_coord(2) - 1.5625))<=0.000001 ) then
                tmp_val = (M(k)/(epsi*epsi*epsi)) * grad_phi
!~ 				open (unit=out_unit,file="sp.log",action="write", position="append")
!~ 	            write (out_unit,*) k, X_coord, tmp_val, grad_phi
!~ 	            close (out_unit)            	
            end if
        end do
    ...
    !-------------------------------------------------------
    subroutine grad_convolution(grad_convol, epsi, X_coord, Xgrid_convol, Ygrid_convol, fct_grid_convol, &
    quadrature_pts_convol, nqx, nqy)
        integer, intent(in)                                  :: nqx, nqy
        double precision, dimension (2), intent(in)          :: X_coord        
        double precision, intent(in)                         :: epsi
        double precision, dimension (nqx,nqy), intent(in)    :: quadrature_pts_convol, fct_grid_convol
        double precision, dimension (nqx), intent(in)        :: Xgrid_convol
        double precision, dimension (nqy), intent(in)        :: Ygrid_convol
        ...
        grad_phi = 0.
        grad_convol=0.
        dx = ABS( Xgrid_convol(2) - Xgrid_convol(1))
        dy = ABS( Ygrid_convol(2) - Ygrid_convol(1))
        do i=1,nqx
            do j=1,nqy
                call grad_phi_2d(grad_phi, (X_coord(1)-Xgrid_convol(i))/epsi , (X_coord(2)-Ygrid_convol(j))/epsi )
                grad_phi = (1./(epsi*epsi*epsi)) * grad_phi
                grad_convol=grad_convol + (1/4.)*dx*dy*quadrature_pts_convol(i,j)*fct_grid_convol(i,j) * grad_phi 
            end do
        end do
        
    end subroutine

    subroutine hess_convolution(hess_convol, epsi, X_coord, Xgrid_convol, Ygrid_convol, fct_grid_convol, &
    quadrature_pts_convol, nqx, nqy)
        integer, intent(in)                                  :: nqx, nqy
        double precision, dimension (2), intent(in)          :: X_coord        
        double precision, intent(in)                         :: epsi
        ...
        double precision, dimension (2,2), intent(out)       :: hess_convol
        ...
        do i=1,nqx
            do j=1,nqy
                call hess_phi_2d(hess_phi, (X_coord(1)-Xgrid_convol(i))/epsi , (X_coord(2)-Ygrid_convol(j))/epsi )
                hess_phi = (1./(epsi*epsi*epsi*epsi)) * hess_phi
                hess_convol = hess_convol + (1/4.)*dx*dy*quadrature_pts_convol(i,j) * fct_grid_convol(i,j) * hess_phi 
            end do
        end do
	...
	subroutine diffusion_field_ltp(Xpart, M, DD, hx_remap, hy_remap, diff_field, N_part)
        integer, intent(in)                         :: N_part 
        double precision, dimension (2,N_part), intent(in)    :: Xpart
        double precision, dimension (N_part), intent(in)      :: M
        double precision, dimension (4, N_part), intent(in)   :: DD
    ...
    subroutine diffusion_field_sp(Xpart, M, epsi,  diff_field, N_part)
         integer, intent(in)                         :: N_part
        double precision, dimension (2,N_part), intent(in)    :: Xpart
        double precision, dimension (N_part), intent(in)      :: M                
        double precision, dimension (2,N_part), intent(out)   :: diff_field
...
!~ 	        !$OMP PARALLEL DO shared(chunk) private(j, rho, grad_rho) schedule(dynamic,chunk)
	        do j=1,N_part                        
	            ...
	            call rec_sp_grad_at_coordinate(grad_rho, Xpart, M, epsi, Xpart(:,j), N_part)	
	            diff_field(1,j) = m_barenblatt * grad_rho(1)
	            diff_field(2,j) = m_barenblatt * grad_rho(2)
	        end do
!~ 	        !$OMP END PARALLEL DO    
        else 
!~ 	        !$OMP PARALLEL DO shared(chunk) private(j, rho, grad_rho) schedule(dynamic,chunk)
...
!        !$OMP END PARALLEL DO
    ...
    Xgrid_convol, Ygrid_convol, quadrature_pts_convol, diff_field, nqx, nqy, N_part)
        integer, intent(in)                         :: N_part, nqx, nqy, m_barenblatt
        double precision, dimension (2,N_part), intent(in)    :: Xpart
        double precision, dimension (N_part), intent(in)      :: M
        double precision, dimension (4, N_part), intent(in)   :: DD
        double precision, intent(in)                          :: hx_remap, hy_remap, epsi
        ...
        double precision, dimension (2)       :: result_grad_convol
...
!~             do ii=1,nqx
!~                 do jj=1,nqy 
!~                     if (abs(rho_ltp_conv_grid(ii,jj)) > 0.) then
!~                         print *, "rho_ltp_conv_grid(",ii,",",jj,") = " , rho_ltp_conv_grid(ii,jj)
!~                     end if
!~                 end do
!~             end do
            
            do j=1,N_part                                
                call grad_convolution(result_grad_convol, epsi, Xpart(:,j), Xgrid_convol, Ygrid_convol, &
                rho_ltp_conv_grid, quadrature_pts_convol, nqx, nqy)
                diff_field(1,j) = m_barenblatt * result_grad_convol(1)
                diff_field(2,j) = m_barenblatt * result_grad_convol(2)                
                print *, "fortran : j , diff_field(1,j), diff_field(2,j) = " , j , diff_field(1,j), diff_field(2,j)
            end do
        else
