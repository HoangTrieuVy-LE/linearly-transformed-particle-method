...
Nboucle_total = (config.Tmax - config.Tini)/config.dt
myfile=open('./particles_init.debug','w')
for k in range(0,N) :
    myfile.write(str(k)+"\t"+str(X[0,k])+"\t"+str(X[1,k])+'\n')
myfile.close()			



#~ print "config.hx_remap , config.hy_remap, config.epsilon : " , config.hx_remap , config.hy_remap, config.epsilon
#~ print "config.method , config.D_method, config.time_scheme : " ,config.method , config.D_method, config.time_scheme
#~ print "config.C, config.alpha , config.beta, config.k, config.d : " ,config.C, config.alpha , config.beta, config.k, config.d
start1=time.clock()
...
if ((config.method=='LTP') | (config.method == 'analytic_debug')):
    R_ltp = reconstruction_densites_2D.rec_densite_grille_unif_LTP(X, M, D, Xgrille, Ygrille)
    npicf_ltp = 3000 + npic
